...
a binary-state model with $K\geq 3$ groups, the CARA market-clearing
condition aggregates private posteriors in log-odds for arbitrary
heterogeneity in risk aversion, signal precision, and wealth, and is fully
revealing of the Bayesian sufficient statistic when risk aversion is
homogeneous (Proposition~\ref{prop:cara}). Heterogeneous risk aversion
introduces a separate route to partial revelation, but the underlying
log-odds aggregation is robust and continues to identify CARA as the
boundary case. Second, I show that under any CRRA
