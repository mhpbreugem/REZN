...
println("="^127)
@printf "%3s | %-10s | %5s | %8s | %10s | %10s | %9s | %15s | %15s | %8s\n" "G" "method" "iters" "time(s)" "alloc(MB)" "peakRSS(MB)" "‖F‖∞" "p*" "μ₁" "PR_gap"
println("-"^127)
for (sp, sn, jmb) in results
    @printf "%3d | %-10s | %5d | %8.2f | %10s | %10.0f | %9.2e | %15.10f | %15.10f | %8.5f\n" sp.G sp.method sp.iters sp.dt fmt_mb(sp.bytes) sp.rss_mb sp.Finf sp.p sp.mus[1] sp.mus[1]-sp.mus[2]
    if sn !== nothing
        @printf "%3d | %-10s | %5d | %8.2f | %10s | %10.0f | %9.2e | %15.10f | %15.10f | %8.5f\n" sn.G sn.method sn.iters sn.dt fmt_mb(sn.bytes) sn.rss_mb sn.Finf sn.p sn.mus[1] sn.mus[1]-sn.mus[2]
    end
    println("-"^127)
end
