"""Contour plot using mp300 iter 1 μ (max-residual 7.1e-110)."""
import json, time
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
print("Loading mp300 iter 1 μ...", flush=True)
with open(f"{RESULTS_DIR}/posterior_v3_G15_mp300_iter1.json") as f:
    state = json.load(f)
mu_REE = np.array([[float(state["mu_strings"][i][j]) for j in range(G)]
                       for i in range(G)])
print(f"  F_max from json: {state['F_max']}", flush=True)

ck = np.load(f"{RESULTS_DIR}/posterior_v3_strict_G15.npz")
u_grid_R = ck["u_grid"]; p_grid_R = ck["p_grid"]
...
ax.set_title(f"CRRA REE contours (mp300 Newton, max=7.1e-110)\n"
              f"$u_1={u1_fixed}$, $\\tau={TAU}$, $\\gamma={GAMMA}$, $G=15$",
              fontsize=10)
plt.tight_layout()
out = f"{RESULTS_DIR}/fig_contours_mp300"
fig.savefig(f"{out}.pdf", bbox_inches="tight")
fig.savefig(f"{out}.png", dpi=150, bbox_inches="tight")
plt.close(fig)
print(f"Saved {out}.{{pdf,png}}", flush=True)

with open(f"{RESULTS_DIR}/fig_contours_mp300_data.json", "w") as f:
    json.dump({"figure": "fig_contours_mp300",
                "params": {"u1": u1_fixed, "tau": TAU, "gamma": GAMMA,
                           "G": 15, "source": "mp300 NK iter 1",
                           "F_max": "7.1e-110"},
                "p_levels": P_LEVELS,
