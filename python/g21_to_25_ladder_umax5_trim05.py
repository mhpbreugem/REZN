"""Smart ladder mp300 UMAX=5 trim=0.05, G=21..25 starting from G=20 ckpt.

For G in [21, 22, 23, 24, 25]:
  Warm from previous G mp result.
  Float64 picard polish, then mp300 NK with LM + Picard fallback.
  Save mp300 ckpt.
"""
...
TAU = mpf("2"); GAMMA = mpf("0.5"); UMAX = 5.0; TRIM = 0.05
H_FD = mpf("1e-100")
TARGET = mpf("1e-100")
GS = list(range(21, 26))   # 21..25 (resume from G=20 UMAX=5 trim=0.05 ckpt)
...
# Resume from G=20 UMAX=5 trim=0.05 mp300 ckpt
print(f"Loading G=20 UMAX=5 trim=0.05 mp300 ckpt...", flush=True)
with open(f"{RESULTS_DIR}/posterior_v3_G20_umax5_trim05_mp300.json") as f:
    state = json.load(f)
G_PREV = 20
...
for G in GS:
    ...
    with open(out, "w") as f:
        json.dump({"G": G, "UMAX": UMAX, "tau": 2.0, "gamma": 0.5,
                    "trim": TRIM, "dps": 300,
