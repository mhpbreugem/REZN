"""From G=21 trim90 strict seed, try 95% and 100% coverage."""
...
UMAX = 4.0; G = 21; TAU = 2.0; GAMMA = 0.5; TOL_MAX = 1e-14
...
def solve(trim, mu_warm, u_warm, p_warm, label=""):
    ...
    if d_p["max"] < TOL_MAX and d_p["u_viol"] == 0 and d_p["p_viol"] == 0:
        return mu, d_p, u_grid, p_grid, p_lo, p_hi, "strict_picard"
    ...
    if d_nk["max"] < TOL_MAX and d_nk["u_viol"] == 0 and d_nk["p_viol"] == 0:
        return mu_nk, d_nk, u_grid, p_grid, p_lo, p_hi, "strict_NK"
    if d_nk["u_viol"] == 0 and d_nk["p_viol"] == 0:
        return mu_nk, d_nk, u_grid, p_grid, p_lo, p_hi, "non_strict_monotone"
    return mu, d_p, u_grid, p_grid, p_lo, p_hi, "fallback_picard"


warnings.filterwarnings("ignore", category=RuntimeWarning)
print(f"=== G={G} from trim90 strict seed → 95%, 100% ===\n", flush=True)
ck = np.load(f"results/full_ree/posterior_v3_trim90_G{G}.npz")
mu_w, u_w, p_w = ck["mu"], ck["u_grid"], ck["p_grid"]

# 95%
print(f"--- G={G}, c=95% (trim=0.025) ---", flush=True)
mu, d, ug, pg, plo, phi_, status = solve(0.025, mu_w, u_w, p_w, label="c=95")
...
if status.startswith("strict") or status == "non_strict_monotone":
    np.savez(f"results/full_ree/posterior_v3_G{G}_c95.npz",
             mu=mu, u_grid=ug, p_grid=pg, p_lo=plo, p_hi=phi_)
    mu_w95, u_w95, p_w95 = mu, ug, pg
else:
    mu_w95, u_w95, p_w95 = mu_w, u_w, p_w

# 100% — try from c=95% if that worked, else from trim90
print(f"\n--- G={G}, c=100% (trim=0) ---", flush=True)
mu, d, ug, pg, plo, phi_, status = solve(0.0, mu_w95, u_w95, p_w95,
                                            label="c=100")
r2, slope, _ = measure_R2(mu, ug, pg, plo, phi_, TAU, GAMMA)
print(f"  ===> 1-R²={r2:.4e}, slope={slope:.4f}, status={status}",
      flush=True)
