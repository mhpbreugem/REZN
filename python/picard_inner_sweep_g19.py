"""
Inner-domain sweep at G=19: inner halfwidth in {2, 3, 4, 5, 6, 7, 8} sigma.
Buffer always 2 sigma extra each side. CRRA gamma=0.5, tau=2.

Each run: 15 undamped Picard iterations. Plots saved per level.
"""
...
PLOTS_ROOT = os.path.join(os.path.dirname(__file__), "plots_inner_sweep_g19")
...
def plot_contours(level, P_ext, u_inner, u_ext, R2, slope, delta, path, inner_sigmas):
    G_ext = len(u_ext)
    i_mid = G_ext // 2
    fig, axes = plt.subplots(1, 3, figsize=(15, 5.5))
    fig.suptitle(
        f"inner=±{inner_sigmas}σ  Level={level}  slope={slope:.4f}  "
        f"1-R²={R2:.6f}  ||ΔP||∞={delta:.3e}  (G={G_INNER}, CRRA γ={GAMMA}, τ={TAU}, +2σ buffer)",
        fontsize=11,
    )
...
def run_for_inner(inner_sigmas, maxlevel=15, tol=1e-7):
    ...
    print(f"\n=== inner=±{inner_sigmas}σ  G={G_INNER}  G_ext={G_ext}  "
          f"spacing={u_inner[1]-u_inner[0]:.4f}  plots={plots_dir} ===", flush=True)

    t0 = time.time()
    P_ext = build_P_ext(u_ext, G_ext)
    print(f"inner={inner_sigmas}σ  build_P_ext: {time.time() - t0:.1f}s ({G_ext**3} cells)",
          flush=True)

    P_inner = P_ext[np.ix_(inner_idx, inner_idx, inner_idx)].copy()
    R2_0, slope_0 = deficit(P_inner, u_inner)
    history = [(0, 0.0, R2_0, slope_0)]
    print(f"inner={inner_sigmas}σ  LEVEL  0  1-R²={R2_0:.6f}  slope={slope_0:.4f}  (no-learn)",
          flush=True)
...
def plot_summary(all_histories, path):
    ...
    fig.suptitle(
        f"Inner-halfwidth sweep at G={G_INNER}: CRRA γ={GAMMA}, τ={TAU}, +2σ buffer",
        fontsize=11,
    )
    ...
    for s in inner_keys:
        ...
        ax.semilogy(ks, R2s, "o-", label=f"±{s}σ")
    ...
    for s in inner_keys:
        ...
        ax.plot(ks, slopes, "o-", label=f"±{s}σ")
