"""Cutoff homotopy for higher G (20..30), runs after continue_g20_to_30
generates the trim90 seeds.

Also re-tries G=12 at all coverage levels via homotopy (not just 92%).
"""
...
GS = list(range(20, 31))   # 20..30 — append to lower-G results
...
def solve(G, trim, mu_warm, u_warm, p_warm,
...
print(f"=== Cutoff homotopy 90→100% for high G={GS} ===\n", flush=True)
# Read or initialize results
out_path = "results/full_ree/cutoff_homotopy.json"
all_results = {}
...
for G in GS:
    ...
    if not os.path.exists(seed_path):
        print(f"\nSkipping G={G}: trim90 seed not yet ready", flush=True)
        continue
    print(f"\n=== G = {G} ===", flush=True)
    ...
    for trim in TRIMS:
        ...
        if status.startswith("strict") or status == "non_strict_monotone":
            ...
            print(f"    failed at c={coverage_pct:.0f}%; stopping ladder for G={G}",
                  flush=True)
            break
