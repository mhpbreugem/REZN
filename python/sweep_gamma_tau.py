"""(γ, τ) sweep at G=12, smooth Φ (fast), h=0.005.

Strict tolerance: every cell must reach residual ≤ 1e-13. Uses continuation:
each new (γ,τ) is seeded from the previous converged tensor in the snake order
(τ outer loop, γ inner). If a rung doesn't converge in the first attempt it
retries with smaller damping; if still not, halts the sweep at that cell.

Outputs:
  results/full_ree/sweep_gamma_tau_G12_h0.005_{rows.json, table.csv}
"""

import json, subprocess, sys, time, csv
from pathlib import Path
import numpy as np

REPO = Path(__file__).resolve().parent.parent
RESULTS = REPO / "results" / "full_ree"
SOLVER = REPO / "python" / "full_ree_solver_het_smooth_fast.py"
G = 12; H = 0.005

GAMMAS = [0.1, 0.25, 0.5, 1.0, 2.0, 5.0, 20.0]
TAUS   = [0.5, 1.0, 2.0, 3.0, 5.0]
ACCEPT_TOL = 1.0e-13


def call(label, gammas, tau, seed, max_iter=800, tol=1e-14,
         damping=0.3, anderson=5, anderson_beta=0.7):
    cmd = [
        "python3", str(SOLVER),
        "--G", str(G), "--umax", "2", "--tau", str(tau),
        "--gammas", ",".join(f"{g:g}" for g in gammas),
        "--seed-array", str(seed),
        "--label", label, "--h", str(H),
        "--max-iter", str(max_iter), "--tol", str(tol),
        "--damping", str(damping),
        "--anderson", str(anderson), "--anderson-beta", str(anderson_beta),
        "--symmetric",
    ]
    out = subprocess.run(cmd, capture_output=True, text=True)
    if out.returncode != 0:
        sys.stdout.write(out.stdout[-1000:] + "\nSTDERR:\n" + out.stderr[-1000:])
        return None
    g_str = "_".join(f"{g:g}" for g in gammas)
    sf = RESULTS / f"G{G}_tau{tau:g}_smoothfasth{H:g}_het{g_str}_{label}_summary.json"
    if not sf.exists():
        return None
    with open(sf) as f:
        return json.load(f)


def best_resid(s):
    return float(s.get("residual_inf", float("inf"))) if s else float("inf")


def attempt(label, gammas, tau, seed, attempt_idx):
    if attempt_idx == 0:
        kw = dict(max_iter=600, tol=1e-14, damping=0.3, anderson=5, anderson_beta=0.7)
    elif attempt_idx == 1:
        kw = dict(max_iter=2000, tol=1e-14, damping=0.2, anderson=5, anderson_beta=0.7)
    else:
        kw = dict(max_iter=4000, tol=1e-14, damping=0.1, anderson=8, anderson_beta=0.5)
    return call(f"{label}_a{attempt_idx}", gammas, tau, seed, **kw)


def find_seed_path(gammas, tau, label):
    g_str = "_".join(f"{g:g}" for g in gammas)
    return RESULTS / f"G{G}_tau{tau:g}_smoothfasth{H:g}_het{g_str}_{label}_prices.npz"


def main():
    # First-row seed: the converged G=12, γ=2, τ=2 tensor we already have
    initial_seed = RESULTS / f"G{G}_tau2_smoothfasth{H:g}_het2_2_2_g12ladder_g2_prices.npz"
    if not initial_seed.exists():
        # Fall back to bare cursor seed
        initial_seed = RESULTS / "G6_tau2_gamma2_up_picard_0_prices.npz"
    print(f"Initial seed: {initial_seed.name}")

    rows = []
    cur_seed = initial_seed
    print(f"\n{'τ':>5} {'γ':>6} {'iters':>5} {'resid':>11} {'1-R²':>11} {'slope':>7} {'time(s)':>8}")
    print("-" * 70)
    for tau in TAUS:
        first_in_tau = True
        for gam in GAMMAS:
            gammas = (gam, gam, gam)
            label = f"sw_t{tau:g}_g{gam:g}"
            accepted = None
            elapsed_total = 0.0
            for k in range(3):
                t0 = time.time()
                s = attempt(label, gammas, tau, cur_seed, k)
                elapsed = time.time() - t0
                elapsed_total += elapsed
                if s is None: continue
                resid = best_resid(s)
                if resid <= ACCEPT_TOL:
                    accepted = s
                    break
            if accepted is None:
                resid_s = f"{resid:.2e}" if 's' in dir() and s else "FAILED"
                print(f"{tau:>5g} {gam:>6g} HALT (best={resid_s})")
                break
            seed_label = f"{label}_a0"  # the attempt that succeeded was 0; in retry uses different label
            # Find which attempt's npz to use
            for k in range(3):
                p = find_seed_path(gammas, tau, f"{label}_a{k}")
                if p.exists() and best_resid(accepted) <= ACCEPT_TOL:
                    cur_seed = p
                    break
            iters = accepted["iterations"]
            r = accepted["residual_inf"]
            R2 = accepted["revelation_deficit"]
            sl = accepted["slope"]
            print(f"{tau:>5g} {gam:>6g} {iters:>5} {r:>11.3e} {R2:>11.4e} {sl:>7.4f} {elapsed_total:>8.1f}",
                  flush=True)
            rows.append({"tau": tau, "gamma": gam, **accepted})
        else:
            # If all γ in this τ converged, jump back to start γ=2, τ=2 seed
            # for the next τ row's first cell to avoid drift
            continue
        break  # halt outer if inner halted

    # Save
    with open(RESULTS / f"sweep_gamma_tau_G{G}_h{H:g}.json", "w") as f:
        json.dump(rows, f, indent=2)
    csv_path = RESULTS / f"sweep_gamma_tau_G{G}_h{H:g}.csv"
    with open(csv_path, "w") as f:
        w = csv.writer(f)
        w.writerow(["tau", "gamma", "iters", "residual_inf", "1-R2", "slope",
                    "max_FR_err", "price_canon"])
        for r in rows:
            rep = r.get("representative_realization", {})
            w.writerow([r["tau"], r["gamma"], r["iterations"], r["residual_inf"],
                        r["revelation_deficit"], r["slope"], r["max_fr_error"],
                        rep.get("price")])
    print(f"\nSaved {len(rows)} rows to {csv_path}")

    # Print 2D table
    if rows:
        print("\n2D 1-R² table (rows=τ, cols=γ):")
        hdr = "τ\\γ"
        print(f"{hdr:>8}", end="")
        for g in GAMMAS:
            print(f"{g:>10g}", end="")
        print()
        for tau in TAUS:
            print(f"{tau:>8g}", end="")
            for gam in GAMMAS:
                hit = next((r for r in rows if r["tau"] == tau and r["gamma"] == gam), None)
                if hit:
                    print(f"{hit['revelation_deficit']:>10.4e}", end="")
                else:
                    print(f"{'--':>10}", end="")
            print()


if __name__ == "__main__":
    main()
