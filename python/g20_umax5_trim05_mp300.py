"""G=20, UMAX=5, trim=0.05 (keep 90%), mp300 LM with Picard fallback.
Cold start → float64 polish → mp300 NK. Target 1e-100."""
...
G = 20; UMAX = 5.0; TRIM = 0.05  # keep central 90%
...
# Build grid UMAX=5
print(f"Building G={G} UMAX={UMAX} trim={TRIM} grid...", flush=True)
u_grid_np = np.linspace(-UMAX, UMAX, G)
p_lo_np, p_hi_np, p_grid_np = init_p_grid_f64(u_grid_np, 2.0, 0.5, G,
                                                   trim=TRIM)
print(f"  p-range: [{p_lo_np.min():.4f}, {p_hi_np.max():.4f}]", flush=True)

# Cold start: no learning
print(f"Cold start with no-learning Λ(τu)...", flush=True)
mu_f = np.zeros((G, G))
for i, u in enumerate(u_grid_np):
    mu_f[i, :] = Lam_f64(2.0 * u)
mu_f = pava_2d_f64(mu_f)

# Float64 picard polish
print(f"Float64 picard polish...", flush=True)
...
print(f"\nLM mp300 NK at G={G} UMAX={UMAX}...", flush=True)
history = []
lam = mpf("1e-30")
for nk_iter in range(1, 16):
    t_iter = time.time()
    F_curr = F_mu(mu, u_grid, p_grid, p_lo, p_hi, TAU, GAMMA)
    F_max_c = F_max(F_curr); F_med_c = F_med(F_curr)
    F_norm_c = sum(abs(F_curr[i][j])**2 for i in range(G) for j in range(G))
    ...
    for attempt in range(8):
        ...
        for k in range(n):
            ...
            if mu_trial[i][j] < mpf("1e-150"): mu_trial[i][j] = mpf("1e-150")
        ...
        if F_norm_try < F_norm_c:
            ...
            F_norm_c = F_norm_try
            print(f"    LM accepted, new λ={mpmath.nstr(lam, 3)}", flush=True)
            break
        else:
            lam = lam * mpf(10)
            print(f"    LM rejected, λ={mpmath.nstr(lam, 3)}", flush=True)
    if not accepted:
...
# Save
mu_strs = [[mpmath.nstr(mu[i][j], 200) for j in range(G)] for i in range(G)]
out = f"{RESULTS_DIR}/posterior_v3_G{G}_umax5_trim05_mp300.json"  # auto G=20
with open(out, "w") as f:
    json.dump({"G": G, "UMAX": UMAX, "tau": 2.0, "gamma": 0.5, "trim": TRIM,
                "dps": 300,
