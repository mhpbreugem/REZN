"""Strict ladder G=18,19,20,21,22,23,24,25 with p-grid trim.
Single G-step warm-start chain from G=17 trim strict.
"""
...
UMAX = 4.0; TAU = 2.0; GAMMA = 0.5; TOL_MAX = 1e-14; TRIM = 0.025
...
def solve_one(G, mu_warm, u_warm, p_warm, label=""):
    ...
    mu_picard = mu
    d_p = measure(mu_picard, u_grid, p_grid, p_lo, p_hi)
    ...
    try:
        ...
        mu_nk = mu_picard; nk_status = f"err:{type(exc).__name__}"
...
print(f"=== G ladder 18..25 with trim={TRIM} ===\n", flush=True)
results = []
ck = np.load("results/full_ree/posterior_v3_trim_G17.npz")
mu_w, u_w, p_w = ck["mu"], ck["u_grid"], ck["p_grid"]
print(f"Seed: G=17 trim strict\n", flush=True)

for G in GS:
    print(f"--- G = {G} ---", flush=True)
    mu, d, ug, pg, plo, phi_, status, elapsed = solve_one(
        G, mu_w, u_w, p_w, label=f"G={G}")
    r2, slope, _ = measure_R2(mu, ug, pg, plo, phi_, TAU, GAMMA)
    print(f"  ===> 1-R²={r2:.4e}, slope={slope:.4f}, status={status}",
          flush=True)
    rec = {"G": G, **d, "1-R^2": float(r2), "slope": float(slope),
           "status": status, "elapsed_s": elapsed}
    results.append(rec)
    # Update warm if monotone (strict or near-strict)
    ...
    with open("results/full_ree/strict_trim_18_25.json", "w") as f:
