"""Full strict ladder G=10..30 with p-grid trim=0.05 (central 90%).
Single G-step warm-start chain. Strict 1e-14 with monotonicity required.
"""
...
UMAX = 4.0; TAU = 2.0; GAMMA = 0.5; TOL_MAX = 1e-14; TRIM = 0.05
GS = list(range(10, 31))   # 10..30
...
def solve(G, mu_warm, u_warm, p_warm, label=""):
...
print(f"=== Strict G ladder 10..30 with trim={TRIM} (central {100-2*TRIM*100:g}%) ===\n",
      flush=True)
results = []
mu_w = u_w = p_w = None  # cold start at G=10

for G in GS:
    print(f"--- G = {G} ---", flush=True)
    mu, d, ug, pg, plo, phi_, status, elapsed = solve(
    ...
    with open("results/full_ree/strict_trim90_ladder.json", "w") as f:
