...
# Warm: G=17 mp200 → G=50 interp (in mpmath)
print(f"Loading G=17 mp200 warm...", flush=True)
with open(f"{RESULTS_DIR}/posterior_v3_G17_mp200.json") as f:
    state17 = json.load(f)
G17 = 17
mu_17_strs = state17["mu_strings"]
u_17_strs = state17["u_grid"]
p_17_strs = state17["p_grid"]
mu_17 = [[mpf(mu_17_strs[i][j]) for j in range(G17)] for i in range(G17)]
u_17 = [mpf(s) for s in u_17_strs]
p_17 = [[mpf(p) for p in row] for row in p_17_strs]
...
# Interp G=17 → G=50 in mp
print(f"Interpolating G=17 → G=50 in mp300...", flush=True)
mu = []
for i in range(G):
    row = []
    u = u_grid[i]
