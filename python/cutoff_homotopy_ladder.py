"""Cutoff homotopy ladder: widen coverage from 90% to 100% in 1% steps.

For each G in {13, 14, 15, 16, 17, 18, 19}:
  Start from trim=0.05 (90%) strict. Widen by 0.005 per step (1% wider).
  Warm-start each step from previous coverage.
  Track at which coverage strict fails.
"""
...
GS = [13, 14, 15, 16, 17, 18, 19]
# Trim ladder: 0.05 (90%), 0.045 (91%), 0.04 (92%), ..., 0.0 (100%)
TRIMS = [0.05, 0.045, 0.04, 0.035, 0.03, 0.025, 0.02, 0.015,
         0.010, 0.005, 0.0]
...
print(f"=== Cutoff homotopy 90→100% for G={GS} ===\n", flush=True)
all_results = {}
...
for G in GS:
    ...
    try:
        ...
        print(f"\nSkipping G={G}: no trim90 checkpoint", flush=True)
        continue
    print(f"\n=== G = {G} ===", flush=True)
    ...
    for trim in TRIMS:
        ...
        label = f"G={G}, c={coverage_pct:.0f}%"
        mu, d, ug, pg, plo, phi_, status, t = solve(
            G, trim, mu_w, u_w, p_w, label=label)
        r2, slope, _ = measure_R2(mu, ug, pg, plo, phi_, TAU, GAMMA)
        rec = {"G": G, "trim": trim, "coverage_pct": coverage_pct,
               **d, "1-R^2": float(r2), "slope": float(slope),
               "status": status, "t": t}
        all_results[str(G)].append(rec)
        print(f"  c={coverage_pct:>3.0f}%: max={d['max']:.2e} "
              f"med={d['med']:.2e} u/p={d['u_viol']}/{d['p_viol']} "
        ...
        if status.startswith("strict") or status == "non_strict_monotone":
            ...
            print(f"    [{label}] failed strict; stopping ladder for G={G}",
                  flush=True)
            break
        with open("results/full_ree/cutoff_homotopy.json", "w") as f:
