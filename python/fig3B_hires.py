...
# Save
with open(f"{RESULTS_DIR}/fig_multicontour_B_hires_data.json", "w") as f:
    json.dump({"figure": "fig_multicontour_B_hires",
                "params": {"u1": u1_fixed, "tau": TAU, "gamma": GAMMA,
                           "model": "CRRA_REE", "G": 15, "N_grid": N},
                "p_levels": P_LEVELS,
...
with open(f"{RESULTS_DIR}/fig_multicontour_B_hires_pgfplots.tex", "w") as f:
    for p in P_LEVELS:
        f.write(f"% p={p}\n")
        f.write(f"\\addplot coordinates "
                f"{{{pgf(CRRA_contours[f'{p:g}'])}}};\n\n")
print("Saved fig_multicontour_B_hires_*.{json,tex}", flush=True)
...
ax.set_title(f"Fig 3B hires: CRRA contours (REE)\n"
              ...
              fontsize=10)
plt.tight_layout()
fig.savefig(f"{RESULTS_DIR}/fig_multicontour_B_hires_preview.pdf",
              bbox_inches="tight")
fig.savefig(f"{RESULTS_DIR}/fig_multicontour_B_hires_preview.png",
